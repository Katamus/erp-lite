...
import com.criscahub.erp_lite.persistence.mongo.mappers.ProductCatalogMapper;
import com.criscahub.erp_lite.persistence.mongo.repositories.ProductInCatalogRepository;
...
public class ProductCatalogRepositoryAdapter implements ProductCatalogRepositoryPort {
    ...
    public List<ProductView> findActive() {
        ...
        if (raw != null) {
            log.debug("Found product with active");
            return this.objectMapper.convertValue(raw,
                    this.objectMapper.getTypeFactory().constructCollectionType(List.class,ProductView.class));
        }
        log.debug("Finding product with active");
